	...
	public function get_archieves_vital_json()
	{
		...
		if (!empty($archieves)) {
			foreach ($archieves as $archieve) {
				...
				$btn_detail = '<a href="' . base_url('v2/backend/alih_media_arsip_vital/detail?') . http_build_query($params) . '" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-eye"></i></a>';
				...
				$nested['klasifikasi'] = '<a href="'. base_url('v2/backend/alih_media_arsip_vital/detail?') . http_build_query($params) . '" class="text-primary">' . $archieve->kode_klsf ?? '-' . '</a>';
