          ...
          <div class="col-12">
               <div class="card">
                    <div class="card-header">
                         ...
                         <button class="btn btn-primary btn-sm" onclick="add_galeri()"><i class="fas fa-plus me-1"></i> Tambah Data</button>
                    </div>
                    <div class="card-body">
...
$(document).ready(function() {
     table = $('#dataTable').DataTable({
          ...
          "ajax": { "url": "<?php echo base_url('v2/backend/galeris/ajax_list') ?>", "type": "POST" },
          "columnDefs": [{ "targets": [-1], "orderable": false }],
     });
     $("input, textarea, select").change(function() { $(this).closest('.mb-3').find('.help-block').empty(); $(this).removeClass('is-invalid'); });
     try {
          $('.dropify').dropify({ messages: { default: '<h6>Pilih Gambar Galeri<br>Format: JPG, PNG</h6>', replace: 'Ganti', remove: 'Hapus', error: 'Error' } });
     } catch (e) {
...
function add_galeri() {
     ...
     var myModal = new bootstrap.Modal(document.getElementById('modal_form')); myModal.show(); $('.modal-title').text('Tambah Galeri');
...
function edit_galeri(id) {
     ...
     $.ajax({
          url: "<?php echo site_url('v2/backend/galeris/ajax_edit/') ?>" + id, type: "GET", dataType: "JSON",
          success: function(data) {
               ...
               var myModal = new bootstrap.Modal(document.getElementById('modal_form')); myModal.show(); $('.modal-title').text('Edit Galeri');
          },
...
function save() {
     ...
     var url = (save_method == 'add') ? "<?php echo site_url('v2/backend/galeris/ajax_add') ?>" : "<?php echo site_url('v2/backend/galeris/ajax_update') ?>";
...
function delete_galeri(id) {
     Swal.fire({
          ...
          if (result.isConfirmed) {
               $.ajax({ url: "<?php echo site_url('v2/backend/galeris/ajax_delete/') ?>" + id, type: "POST", dataType: "JSON",
