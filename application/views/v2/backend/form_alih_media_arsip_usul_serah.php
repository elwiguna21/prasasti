...
#smartwizard .nav .nav-item {
    ...
    z-index: 1;
    padding: 0 20px;
    background: #fff; /* Tutup garis jika menumpuk */
}
...
.sw-btn-group { gap: 8px; margin-top: 20px; }
...
#pdf-upload-preview:hover, #pdf-upload-preview.dragover {
     border-color: #5c78ef;
     background: #eef1fb;
}
#pdf-upload-preview .upload-icon { font-size: 48px; color: #adb5bd; margin-bottom: 10px; }
#pdf-upload-preview.has-file { border-color: #28a745; background: #f0fff4; }
#pdf-upload-preview.has-file .upload-icon { color: #28a745; }
</style>
     ...
     <div class="col-12">
          <div class="card">
               <div class="card-header">
                    <h4 class="card-title"><i class="fas fa-file-alt me-2"></i>Form Alih Media Arsip Usul Serah</h4>
               </div>
               <div class="card-body">
                         ...
                         <div class="tab-content">
                              ...
                              <div id="step-2" class="tab-pane" role="tabpanel">
                                   ...
                                   <div class="row justify-content-center">
                                        <div class="col-md-8">
                                             <div id="pdf-upload-preview" onclick="document.getElementById('file_pdf_input').click()">
...
$(document).ready(function() {
     $('#smartwizard').smartWizard({
          ...
          enableURLhash: false,
          transition: { animation: 'fade', speed: '300' },
          toolbarSettings: {
     ...
     dropArea.addEventListener('dragleave', function() { this.classList.remove('dragover'); });
     ...
     interact('#tte-box').draggable({
          ...
          edges: { right: true, bottom: true, bottomRight: '.tte-resize-handle' },
          listeners: {
               move(event) {
                    var target = event.target;
...
function validateStep1() {
     var valid = true;
     var fields = [
          { id: 'kode_klsf',             label: 'Kode Klasifikasi' },
          { id: 'uraian_informasi_arsip', label: 'Uraian Informasi Arsip' },
          { id: 'tahun',                 label: 'Kurun Waktu' },
          { id: 'jumlah',                label: 'Jumlah Dokumen' },
          { id: 'tanggal',               label: 'Waktu (Tanggal)' },
     ];
...
// ===== Submit Form =====
function submitForm() {
     if (!validateStep1()) { alert('Lengkapi data berkas terlebih dahulu (Step 1).'); return; }
     if (!validateStep2()) { alert('Upload file PDF terlebih dahulu (Step 2).'); return; }
     ...
     $.ajax({
          url: base_url + 'v2/backend/alih_media_arsip_usul_serah/ajax_add',
          type: 'POST',
          data: formData,
          contentType: false,
          processData: false,
          ...
          success: function(res) {
               if (res.status) {
                    Swal.fire({
                         title: 'Berhasil!',
                         text: 'Data berkas berhasil disimpan.',
                         icon: 'success',
