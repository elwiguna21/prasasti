          ...
          <div class="col-12">
               <div class="card">
                    <div class="card-header">
                         ...
                         <button class="btn btn-primary btn-sm" onclick="add_banner()"><i class="fas fa-plus me-1"></i> Tambah Data</button>
                    </div>
                    <div class="card-body">
               ...
               <div class="modal-body">
                    <form action="#" id="form" class="form-horizontal">
                         ...
                         <div class="mb-3">
                              ...
                              <input type="text" name="judul" class="form-control" placeholder="Masukkan judul banner">
...
$(document).ready(function() {
     table = $('#dataTable').DataTable({
          ...
          "ajax": { "url": "<?php echo base_url('v2/backend/banners/ajax_list') ?>", "type": "POST" },
          "columnDefs": [{ "targets": [-1], "orderable": false }],
     });
     $("input, textarea, select").change(function() { $(this).closest('.mb-3').find('.help-block').empty(); $(this).removeClass('is-invalid'); });
     try {
          $('.dropify').dropify({ messages: { default: '<h6>Pilih Gambar Banner<br>Format: JPG, PNG</h6>', replace: 'Ganti', remove: 'Hapus', error: 'Error' } });
     } catch (e) {
...
function add_banner() {
     save_method = 'add'; $('#form')[0].reset(); $('#photo-preview div').html(''); $('.help-block').empty();
     var myModal = new bootstrap.Modal(document.getElementById('modal_form')); myModal.show(); $('.modal-title').text('Tambah Banner');
...
function edit_banner(id) {
     ...
     $.ajax({
          url: "<?php echo site_url('v2/backend/banners/ajax_edit/') ?>" + id, type: "GET", dataType: "JSON",
          success: function(data) {
               $('[name="id"]').val(data.id); $('[name="judul"]').val(data.caption);
               if (data.file) { $('#photo-preview div').html('File lama: <img src="' + base_url + 'assets/upload/' + data.file + '" alt="" width="200" class="img-thumbnail"><input type="hidden" name="fileold" value="' + data.file + '">'); }
               var myModal = new bootstrap.Modal(document.getElementById('modal_form')); myModal.show(); $('.modal-title').text('Edit Banner');
          },
...
function save() {
     ...
     var url = (save_method == 'add') ? "<?php echo site_url('v2/backend/banners/ajax_add') ?>" : "<?php echo site_url('v2/backend/banners/ajax_update') ?>";
...
function delete_banner(id) {
     Swal.fire({
          ...
          if (result.isConfirmed) {
               $.ajax({ url: "<?php echo site_url('v2/backend/banners/ajax_delete/') ?>" + id, type: "POST", dataType: "JSON",
