          ...
          <div class="col-12">
               <div class="card">
                    <div class="card-header">
                         ...
                         <button class="btn btn-primary btn-sm" onclick="add_artikel()"><i class="fas fa-plus me-1"></i> Tambah Data</button>
                    </div>
                    <div class="card-body">
     ...
     <div class="modal-dialog modal-lg">
          <div class="modal-content">
               <div class="modal-header">
                    <h5 class="modal-title">Artikel Form</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
               </div>
               <div class="modal-body">
                    <form action="#" id="form" class="form-horizontal">
                         <input type="hidden" value="" name="idartikel" />
                         <div class="mb-3">
                         ...
                         <div class="mb-3">
                              ...
                              <textarea name="isi" class="form-control" rows="6" placeholder="Masukkan isi artikel"></textarea>
                              <span class="help-block text-danger"></span>
                         </div>
...
$(document).ready(function() {
     table = $('#dataTable').DataTable({
          ...
          "order": [2, 'asc'],
          "ajax": {
               "url": "<?php echo base_url('v2/backend/artikels/ajax_list') ?>",
               "type": "POST"
          },
          "columnDefs": [
               {
     ...
     try {
          $('.dropify').dropify({
          messages: {
               default: '<h6>Pilih Gambar. Anda juga bisa drag file kesini<br>Format: JPG, PNG</h6>',
               replace: 'Ganti',
               remove: 'Hapus',
               error: 'Error'
          }
...
function edit_artikel(id) {
     ...
     $.ajax({
          url: "<?php echo site_url('v2/backend/artikels/ajax_edit/') ?>" + id,
          type: "GET",
          dataType: "JSON",
          success: function(data) {
               $('[name="idartikel"]').val(data.idartikel);
...
function save() {
     ...
     var url;
     if (save_method == 'add') {
          url = "<?php echo site_url('v2/backend/artikels/ajax_add') ?>";
     } else {
          url = "<?php echo site_url('v2/backend/artikels/ajax_update') ?>";
     }
...
function delete_artikel(id) {
     Swal.fire({
          ...
          if (result.isConfirmed) {
               $.ajax({
                    url: "<?php echo site_url('v2/backend/artikels/ajax_delete/') ?>" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
