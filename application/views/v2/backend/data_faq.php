...
<script type="text/javascript">
     ...
     table = $('#dataTable').DataTable({
          ...
          "order": [1, 'asc'],
          "ajax": {
               "url": "<?php echo base_url('v2/faqs/ajax_list') ?>",
               "type": "POST"
          },
          "columnDefs": [{
               "targets": [-1],
     ...
     function edit_faq(id) {
          ...
          $('.help-block').empty();
          $.ajax({
               url: "<?php echo site_url('v2/faqs/ajax_edit/') ?>" + id,
               type: "GET",
               dataType: "JSON",
     ...
     function save() {
          ...
          var url = (save_method == 'add') ? "<?php echo site_url('v2/faqs/ajax_add') ?>" : "<?php echo site_url('v2/faqs/ajax_update') ?>";
          $.ajax({
               url: url,
               type: "POST",
               data: $('#form').serialize(),
     ...
     function delete_faq(id) {
          Swal.fire({
               ...
               if (result.isConfirmed) {
                    $.ajax({
                         url: "<?php echo site_url('v2/faqs/ajax_delete/') ?>" + id,
                         type: "POST",
                         dataType: "JSON",
                         success: function() {
