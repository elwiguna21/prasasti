<div class="d-flex flex-column gap-5">
     <div class="blog-widget">
          <div class="h4 fw-bold mb-4">Cari Berita</div>

          <form role="search" method="get">
               <input type="search" placeholder="Cari judul berita..." class="form-control" value="<?= !empty($_GET['keyword']) ? $_GET['keyword'] : ''; ?>" autocomplete="off">
     ...
     <div class="blog-widget">
          ...
          <div class="d-flex flex-column gap-4">
               ...
               <?php if (!empty($newslatters_last)) {
                    foreach ($newslatters_last as $newslatter_last) {
               ?>
                         <div class="widget-blog-post">
                              <div class="blog-thumbnail">
                                   <?php
                                   $file_name = base_url('assets/upload/') . $newslatter_last->gambar;
                                   ?>
                                   <?php if (file_exists('./assets/upload/' . $newslatter_last->gambar)) { ?>
                                        <img src="<?= $file_name; ?>" alt="<?= $newslatter_last->judul ?>">
                                   <?php } else { ?>
                                        <img src="<?= base_url('assets/v3/frontend/v2/') ?>img/bg-img/122.jpg" alt="<?= $newslatter_last->judul; ?>">
                                   <?php } ?>

                              </div>
                              <div class="blog-content">
                                   <p class="mb-1 text-primary"><?= tgl_indo(date('Y-m-d', strtotime($newslatter_last->tanggal))); ?></p>
                                   <a href="<?= base_url('v2/news/detail?slug=' . $newslatter_last->slug); ?>"><?= $newslatter_last->judul; ?></a>
                              </div>
                         </div>
               <?php }
               } ?>
               ...
               <?php if (!empty($articles_last)) {
                    foreach ($articles_last as $article_last) { ?>
                         <div class="widget-blog-post">
                              <div class="blog-thumbnail">
                                   <?php
                                   if (file_exists('./assets/upload/' . $article_last->gambar)) {
                                        $article_last_file     = base_url('assets/upload/') . $article_last->gambar;
                                   } else {
                                        $article_last_file     = base_url('assets/v3/frontend/v2/img/bg-img/122.jpg');
                                   }
                                   ?>
                                   <img src="<?= $article_last_file; ?>" alt="<?= $article_last->judul; ?>">
                              </div>
                              <div class="blog-content">
